...
// ─── Configuração ─────────────────────────────────────────────────────────────
// Troque pelo ID do documento da empresa no Firestore
// (a string longa que aparece como nome do documento em /dados/{ID})
const EMPRESA_ID = "RtpwiebzuNXVl7xVgH78WS7Cqnb2";
...
async function chamarIA(system, user) {
  ...
  return d.content?.[0]?.text || "Não foi possível gerar.";
}
...
Receita últimos 30 dias: ${formatarReal(metricas.receitaRecente)}.
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <input
            ...
            style={{
              ...
              border: "0.5px solid #ddd", outline: "none", fontFamily: "inherit",
            }}
          />
...
export default function App() {
  ...
  const { carregando, erro, clientes, insights, metricas, config } = useCRM(EMPRESA_ID);

  if (carregando) return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100vh", fontSize: 14, color: "#888" }}>
      Conectando ao Assent Gestão...
    </div>
  );

  if (erro) return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100vh", fontSize: 14, color: "#A32D2D" }}>
      {erro}
    </div>
  );
    ...
    <div style={{ display: "flex", height: "100vh", fontFamily: "system-ui, sans-serif", background: "#fafaf9" }}>
      ...
      <div style={{ width: 220, background: "#fff", borderRight: "0.5px solid #e5e5e3", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ padding: "14px 18px", borderTop: "0.5px solid #e5e5e3" }}>
          ...
          <div style={{ fontSize: 11, color: "#888", marginTop: 2 }}>Goiânia · GO</div>
        </div>
      </div>
            ...
            <>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 20 }}>
                {[
                  ...
                  <div key={m.lbl} style={{ background: "#fff", border: "0.5px solid #e5e5e3", borderRadius: 10, padding: 16 }}>
                    <div style={{ fontSize: 22, fontWeight: 600, color: m.cor, fontVariantNumeric: "tabular-nums" }}>{m.val}</div>
              ...
              {insights.length === 0 ? (
                <div style={{ textAlign: "center", padding: "40px 0", color: "#aaa", fontSize: 13 }}>
                  Nenhum insight gerado ainda. Aguardando dados do Assent Gestão.
                </div>
              ) : (
                insights.map((ins) => (
                  <InsightCard key={ins.id} insight={ins} empresaNome={config?.empresaNome} />
                ))
              )}
            </>
          )}
            ...
            <>
              ...
              <div style={{ background: "#fff", border: "0.5px solid #e5e5e3", borderRadius: 10, overflow: "hidden" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                  ...
                  <tbody>
                    {[...clientes].sort((a, b) => {
                      const ord = { alto: 0, medio: 1, baixo: 2, indefinido: 3 };
            ...
            <>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 20 }}>
                {[
                  ...
                  <div key={m.lbl} style={{ background: "#fff", border: "0.5px solid #e5e5e3", borderRadius: 10, padding: 16 }}>
                    <div style={{ fontSize: 22, fontWeight: 600, fontVariantNumeric: "tabular-nums" }}>{m.val}</div>
                    <div style={{ fontSize: 11, color: "#888", marginTop: 6 }}>{m.lbl}</div>
                  </div>
                ))}
              </div>
